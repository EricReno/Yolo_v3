...
class FPN(nn.Module):
    def __init__(self, feat_dims, nblocks):
        ...
        self.reduce_layer_1 = Conv(c4, int(0.5*c4), k=1)

        # P4 -> P3
        self.top_down_layer_2 = ConvBlocks(c4 + int(0.5*c4), int(0.5*c4))
        self.reduce_layer_2 = Conv(c3, int(0.5*c3), k=1)

        # P3
        self.top_down_layer_3 = ConvBlocks(c3 + int(0.5*c3), int(0.5*c3))


        self.out_layers = nn.ModuleList([
                Conv(int(0.5*in_dim), out_dim, k=1) for in_dim in feat_dims])
...
def build_fpn(backbone_cfg, fpn_cfg, feat_dims):
    if backbone_cfg == 'darknet_tiny':
        nblocks = 1
    elif backbone_cfg == 'darknet53':
        nblocks = 3

    if fpn_cfg == 'fpn':
        fpn = FPN(feat_dims, nblocks)
